...
visualizer.py
-------------
Отрисовка сетки‑секторов и меток попаданий.

draw_sector_grid(img, *, draw_center=False) – только «пицца» 8×45°
mark_hits(img, hits)                        – сетка + красные точки с номерами
"""
...
def draw_sector_grid(
    ...
    *,
    ...
    thickness: int = 2,
    draw_center: bool = False,   # <-- по‑умолчанию кружок НЕ рисуем
) -> np.ndarray:
    """
    Рисует 8‑секторную «пиццу» поверх изображения.

    Parameters
    ----------
    center / radius
        Явный центр/радиус. Если None – берутся центр картинки и 48 % min(h,w).
    draw_center
        True  -> рисует маленькую окружность (20 % R).
        False -> только лучи + подписи (то, что нужно сейчас).
    """
    ...
    # при необходимости – центральный кружок (девятка/десятка)
    if draw_center:
...
def mark_hits(
    ...
    vis = draw_sector_grid(img)  # кружок не рисуем
